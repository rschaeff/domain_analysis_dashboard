...
import { CorrectedMolstarViewer, Domain } from '@/components/visualization/CorrectedMolstarViewer';

export default function CorrectedDomainPage() {
  ...
  const handleLoadPdb = () => {
    // Force a re-render by resetting domains and adding them again
    const currentDomains = [...domains];
    setDomains([]);
    setTimeout(() => setDomains(currentDomains), 100);
  };
  
  return (
    <div style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto' }}>
      <h1>ECOD Domain Visualization</h1>
      <p>This example demonstrates protein domain visualization using Molstar.</p>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <p style={{ fontSize: '14px', color: '#666', marginTop: '5px' }}>
          Try: 1cbs, 4hhb, 7jtl, 1tim, 6vxx
        </p>
      </div>
      
      <div style={{ border: '1px solid #ddd', borderRadius: '6px', overflow: 'hidden' }}>
        <CorrectedMolstarViewer
          pdbId={pdbId}
          domains={domains}
          height="500px"
        />
      ...
      <div style={{ 
        ...
        {domains.length > 0 ? (
          ...
          <p style={{ textAlign: 'center', color: '#666', padding: '20px', backgroundColor: 'white', borderRadius: '4px', border: '1px solid #eee' }}>
            No domains defined. Add domains using the form below.
          </p>
        )}
      ...
      <div style={{ 
        ...
        <h3 style={{ margin: '0 0 10px 0', color: '#2980b9' }}>About ECOD Domain Visualization</h3>
        <p>
          This component uses Molstar to visualize protein domains from the Evolutionary Classification of Protein Domains (ECOD) database.
          You can define domains by specifying their chain, start and end residues, and assign colors for easy visualization.
        </p>
        <p style={{ marginTop: '10px' }}>
          The visualization allows researchers to explore domain architecture and structural relationships 
          within proteins, helping to understand evolutionary relationships and protein function.
        </p>
      </div>
    </div>
  );
